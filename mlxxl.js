...
let mlxxlurl = $.getdata('mlxxlurl')
let mlxxlhd = $.getdata('mlxxlhd')
let mlxxlbody = $.getdata('mlxxlbody')
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        mlxxlurlArr.push($.getdata('mlxxlurl'))
        mlxxlhdArr.push($.getdata('mlxxlhd'))
        mlxxlbodyArr.push($.getdata('mlxxlbody'))

        let mlxxlcount = ($.getval('mlxxlcount') || '1');
        for (let i = 2; i <= mlxxlcount; i++) {
            mlxxlurlArr.push($.getdata(`mlxxlurl${i}`))
            mlxxlhdArr.push($.getdata(`mlxxlhd${i}`))
            mlxxlbodyArr.push($.getdata(`mlxxlbody${i}`))
        }
        console.log(
...
function tx(timeout = 0) {
    return new Promise((resolve) => {
        hd = (mlxxlhd).replace(/"Keep-Alive"/g,`"Keep-Alive","Content-Length":"2"`)
        let url = {
