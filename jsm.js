...
CRON自行设置
转盘有50次抽奖次数，脚本默认循环10次  需要手动运行脚本5次
...
!(async () => {
  ...
  for (let i = 0; i < jsmhdArr.length; i++) {
      if (jsmhdArr[i]) {
...
const jsmurl = $request.url
   if(jsmurl)     $.setdata(jsmurl,`jsmurl${status}`)
   $.log(jsmurl)

   const jsmhd = JSON.stringify($request.headers)
   if(jsmhd)    $.setdata(jsmhd,`jsmhd${status}`)
$.log(jsmhd)

const jsmbody = $request.body
if(jsmbody)     $.setdata(jsmbody,`jsmbody${status}`)
  $.log(jsmbody)
