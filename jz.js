...
K = '', $.message = '', DATA = '', XH = '0', COOKIES_SPLIT = '',ddtime = '',wzid = '',spid = '',id = '',$.sy = 0;
...
if ($.isNode() && process.env.jztt_jzttURL) {
    XH = process.env.jztt_XH || "0"; //循环ck
    TXTX = process.env.jztt_TXTX || "0";//提现提醒
    SC = process.env.jztt_SC || "0";//阅读时长
...
function GetCookie() {
    ...
    if ($request.url.indexOf("list?") > -1) {
        ...
        //jztturlVal = $request.jztturl.match(/token=(.*)/)[1]
        jztturlVal =JSON.stringify($request.url).split("token=")[1]
        if (jztturlVal) {
            if (XH == 1) {
                cookie()
    ...
    for (let i = 0; i < jztturlArr.length; i++) {
        ...
        if (jztturlVal ) {
                    ...
                    if($.rw.data.daily_task[2].id == 6 && $.rw.data.daily_task[2].is_finish != 1){
                        ...
                        if (K == `签到🚩`) {
                            ...
                            if ($.sign.code == 0) {
                                ...
                                console.log(`打卡翻倍：${$.sign.msg}，获得金币：${$.sign.data.coin}个\n`);
                        ...
                        if (K == `签到翻倍🚩`) {
                            ...
                            if ($.signfb.code == 0) {
                                ...
                                console.log(`打卡翻倍：${$.signfb.msg}，获得金币：${$.signfb.data.coin}个\n`);
            ...
            if (K == `视频id🚩`) {
                ...
                $.spid = DATA
                if ($.spid.code == 0 && $.spid.data.list[RT(0,20)].id) {

                    console.log(`视频id：${$.spid.data.list[RT(0,20)].id}\n`);
                    //$.message +=`【视频id】：${$.spid.data.list[RT(0,20)].id}\n`;
                    spids = $.spid.data.list[RT(0,20)].id
                    await $.wait(3000)
                }
            }
            ...
            if (K == `看视频15次🚩`) {
                ...
                if ($.ksp15c.code == 0) {
                    ...
                    console.log(`看视频：${$.ksp15c.msg}，【获得金币】：${$.ksp15c.data.coin}个\n`);
                    //$.message +=`【看视频】：${$.ksp15c.msg}，【获得金币】：${$.ksp15c.data.coin}个\n`;
                    $.sy += $.ksp15c.data.coin
                }
            }
        }

        for (let x = 0; x < [RT(10,50)]; x++) {
            $.index = x + 1
            ...
            if (K == `文章id🚩`) {
                ...
                $.wzid = DATA
                if ($.wzid.code == 0 && $.wzid.data.list[RT(0,20)].id) {

                    console.log(`文章id：${$.wzid.data.list[RT(0,20)].id} `);
                    //$.message +=`【文章id】：${$.wzid.data.list[RT(0,20)].id} `;
                    wzids = $.wzid.data.list[RT(0,20)].id
                    await $.wait(3000)
                }
            }
            ...
            if (K == `看文章1🚩`) {
                ...
                if ($.kwz1.code == 0) {
                    ...
                    console.log(`${$.kwz1.data.title}\n`);
                    //$.message +=`${$.kwz1.data.title}\n`;
                    await $.wait(3000)
                }
            }
            ...
            if (K == `看文章3🚩`) {
                ...
                if ($.kwz3.code == 0) {
                    ...
                    console.log(`阅读文章金币：${$.kwz3.data.coin}个\n`);
                  //  $.message +=`【阅读文章金币】：${$.kwz3.data.coin}个\n`;
                    $.sy += $.kwz3.data.coin
                    await $.wait(DD)
                }
            }
            ...
            K = `转发文章🚩`;//转发5次
            console.log($.rw.data.daily_task[4].id)
