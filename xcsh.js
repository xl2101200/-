...
[rewrite_local]
#湘创生活
https://api.csxcsh.com/api/sign url script-request-body https://raw.githubusercontent.com/xl2101200/-/main/xcsh.js
...
//ck
function xcshck() {
   if ($request.url.indexOf("integral") > -1 || $request.url.indexOf("sign") > -1) {
    $.setdata($request.url,'xcshurl')
    $.log(xcshurl)
$.setdata(JSON.stringify($request.headers),'xcshhd')
$.log(xcshhd)
    $.setdata($request.body,'xcshbody')
$.log(xcshbody)
   $.msg($.name,"","获取headers成功！")
    } 
  }    
function xcshqd(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : 'https://api.csxcsh.com/api/sign/integral',
        headers : JSON.parse($.getdata('xcshhd')),
        body : xcshbody,}
