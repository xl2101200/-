...
*/
const $ = new Env('平安好车主');
let hczurl = $.getdata('hczurl')
let hczhd = $.getdata('hczhd')
let hczbody = $.getdata('hczbody')
!(async () => {
...
function hczck() {
   if ($request.url.indexOf("completeTask") > -1) {
    $.setdata($request.url,'hczurl')
    $.log(hczurl)
    $.setdata(JSON.stringify($request.headers),'hczhd')
$.log(hczhd)
    $.setdata($request.body,'hczbody')
$.log(hczbody)
   $.msg($.name,"","平安好车主获取headers成功！")
    }
  }
   
function hczqd(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : 'https://hcz-member.pingan.com.cn/micro-api/cps-activity/know/v1/completeTask',
        headers : JSON.parse($.getdata('hczhd')),
        body : hczbody,}
